...
|-------|--------|-----------|
| 2026-06-30 | — | DATAVALIDATIE-TEGEL F1+B2+F2 + fixes (staging). F1: SelectSystems -> één bron-keuzescherm (7 echte bronsystemen, geen KIK-V CSV), landing-pagina weggewerkt (login->portal->kies-je-bron), Mijn taken naar portal (hoofdpagina), Nieuwe scan = kies-je-bron. Benchmark-labels expliciet KIK-V (referentieontwerp). Route-vangnet: verkeerd-gekozen bron met onherkende bestanden valt terug op de juiste validator (lost ZIB-CSV->'Onbekend bestandstype' op); response geeft echt-gebruikte standard terug, frontend routeert daarop. B2: POST /api/validate/benchmark draait kikv|zib tegen de gecachte fase-1 data (run_cache) zonder her-upload. F2: BenchmarkBar onder het fase-1-resultaat met bron-afhankelijke KIK-V/ZIB-knoppen + inline resultaat. 192 tests groen. Alles op staging. |
| 2026-06-30 | — | DATAVALIDATIE-TEGEL F1 (staging): SelectSystems herbouwd van drie keuze-tegels (KIK-V/ZIB/Algemeen) naar één 'Datavalidatie'-bronkeuzescherm (AFAS Profit / Nedap ONS / KIK-V CSV / EPD-ECD). De bron bepaalt de fase-1 standaard en wordt als source meegestuurd (B1); routing naar de bestaande dashboards + bevindingenlijst blijft. KIK-V-CSV gaat nu naar KIK-V-validatie i.p.v. Algemeen -> de Algemeen-vs-KIK-V-divergentie is ook in de UI weg. KIK-V-profielkeuze (werd al genegeerd door App) verschuift naar de benchmark. Frontend-build schoon, backend 191 tests groen. Mockup van de flow afgestemd met Rene. VOLGT: B2 (/benchmark-endpoint dat KIK-V/ZIB als profiel tegen de gecachte run_cache-data draait) + F2 (gecombineerde, bron-afhankelijke benchmark-balk onder het resultaat). |
| 2026-06-30 | — | DATAVALIDATIE-TEGEL Stap 1 / increment B1 (staging). Bron-gestuurde fase-1: /upload accepteert `source` (afas|ons|kikv_csv|epd_ecd) die de fase-1 standaard bepaalt -> geen verkeerde-route-keuze meer. Te ruime AFAS-fallback in _detect_template aangescherpt (& -> subset): KIK-V-CSV met alleen BSN-kolom wordt niet meer als AFAS 'employees' geclassificeerd (lost de gediagnosticeerde Algemeen-vs-KIK-V-divergentie op). Nieuwe `run_cache.py`: per-gebruiker fase-1 data tijdelijk bewaard (overschreven bij nieuwe scan = wissen; TTL 2u; dataminimaal) t.b.v. de latere benchmark (B2). Frontend-plumbing: uploadFiles stuurt source mee (bronsysteem->source). 3 nieuwe tests; suite 191 groen. Backward compatible. VOLGT: F1 UI-consolidatie (3 keuze-tegels -> één 'Datavalidatie'-flow met bron-keuze + bevindingenlijst) en B2/F2 (benchmark-endpoint + gecombineerde benchmark-balk, bron-afhankelijk). Plan: Rhadix_plan_een_datavalidatie_tegel.docx. |
